...
const VideoTestimonials = () => {
  ...
  return <section className="py-16 relative" id="video-testimonials">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex justify-between">
            <button onClick={prevTestimonial} className="glass-button p-3 rounded-full" aria-label="Previous testimonial">
              <ChevronLeft className="text-white" size={24} />
            </button>
            
            <div className="flex gap-2 items-center">
              {testimonials.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`h-3 w-3 rounded-full transition-all ${index === currentIndex ? 'bg-orange-500 scale-125' : 'bg-orange-500/30'}`} aria-label={`Go to testimonial ${index + 1}`} />)}
            </div>
            
            <button onClick={nextTestimonial} className="glass-button p-3 rounded-full" aria-label="Next testimonial">
